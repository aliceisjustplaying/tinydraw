...
constexpr Measurement kMeasurements[] = {
    ...
    {"flash_instruction_hot", "other", 0U, 1U, kWarmupIterations, flash_instruction_probe,
     prepare_hot},
    // Even state-preserving cache-controller writes can transiently disturb
    // later cache preparation. Keep the matched write cohort after every
    // cache-sensitive measurement so the run envelope stays fail-closed.
