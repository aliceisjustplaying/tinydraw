...
using vector_v2::MaterializationQuality;
...
bool present_overview(Co5300PanelTransport& display, DisplayScheduler& scheduler,
  ...
  for (int y = 0; y < vector_v2::kOverviewHeight; y += kStripRows) {
    const int rows = std::min(kStripRows, vector_v2::kOverviewHeight - y);
    const auto offset = static_cast<std::size_t>(y * vector_v2::kOverviewWidth);
    const auto pixels =
        source.subspan(offset, static_cast<std::size_t>(rows * vector_v2::kOverviewWidth));
    hash = hash_pixels(hash, pixels);
    if (!submit_strip(scheduler, display,
                      {.revision = canvas.current_revision(),
                       .panel_bounds = {0, y, vector_v2::kOverviewWidth, y + rows},
                       .pixels = pixels,
  ...
  std::printf(
      "TINYDRAW_VECTOR_V2_WALK_FRAME zoom=25 x=0 y=0 hash=%08lx pushes=%lu "
...
bool present_fallback(Co5300PanelTransport& display, DisplayScheduler& scheduler,
  ...
  std::uint32_t hash = kFnvOffset;
  scheduler.require_revision(canvas.current_revision());
  for (int panel_y = 0; panel_y < vector_v2::kOverviewHeight; panel_y += kStripRows) {
    const int rows = std::min(kStripRows, vector_v2::kOverviewHeight - panel_y);
    const auto destination =
        strip.first(static_cast<std::size_t>(rows * vector_v2::kOverviewWidth));
    const ViewRequest request{
        .zoom = ZoomLevel::k100Percent,
        .level_pixels = PixelRect{world_x, world_y + panel_y, world_x + vector_v2::kOverviewWidth,
    ...
    if (!stats.has_value() || stats->fallback_pixels != destination.size()) {
      std::printf("TINYDRAW_VECTOR_V2_WALK_FAIL reason=compose x=%d y=%d panel_y=%d\n", world_x,
                  world_y, panel_y);
      return false;
    }
    ...
    if (!submit_strip(scheduler, display,
                      {.revision = canvas.current_revision(),
                       .panel_bounds = {0, panel_y, vector_v2::kOverviewWidth, panel_y + rows},
                       .pixels = destination,
                       .stride = vector_v2::kOverviewWidth})) {
      return false;
    }
  ...
  std::printf(
      "TINYDRAW_VECTOR_V2_WALK_FRAME zoom=100 x=%d y=%d hash=%08lx pushes=%lu "
...
bool present_incremental(Co5300PanelTransport& display, DisplayScheduler& scheduler,
  ...
  for (int panel_y = 0; panel_y < vector_v2::kOverviewHeight; panel_y += kStripRows) {
    ...
    const auto stats = canvas.compose_view(
        {.zoom = ZoomLevel::k100Percent,
         .level_pixels = {0, panel_y, vector_v2::kOverviewWidth, panel_y + rows}},
        destination);
    if (!stats.has_value() || stats->revision != revision) {
      std::printf("TINYDRAW_VECTOR_V2_WALK_FAIL reason=incremental_compose phase=%s panel_y=%d\n",
                  phase, panel_y);
      return false;
    }
  ...
  const bool passed =
      ...
      tile_pixels == 2U * vector_v2::kTilePixels &&
      tile_pixels + fallback_pixels == static_cast<std::size_t>(vector_v2::kOverviewPixels) &&
      display.rejected_push_count() == 0U;
  std::printf(
      "TINYDRAW_VECTOR_V2_WALK_INCREMENTAL phase=%s revision=%lu hash=%08lx pushes=%lu "
...
void fill_tile_from_overview(std::span<std::uint16_t> tile, std::span<const std::uint16_t> overview,
                             TileKey key) {
  const PixelRect bounds = vector_v2::tile_pixel_bounds(key);
  for (int y = bounds.y0; y < bounds.y1; ++y) {
    for (int x = bounds.x0; x < bounds.x1; ++x) {
      const std::size_t destination =
          static_cast<std::size_t>(y - bounds.y0) * vector_v2::kTileWidth +
          static_cast<std::size_t>(x - bounds.x0);
      const std::size_t source = static_cast<std::size_t>(y / 4) * vector_v2::kOverviewWidth +
...
bool append_and_commit_probe(OperationLog& log, MaterializedCanvas& canvas,
                             const vector_v2::OperationAppend& append_request,
  ...
  std::printf("TINYDRAW_VECTOR_V2_WALK_OPERATION revision=%lu append_us=%lld committed=%u\n",
...
void run_vector_v2_overview_walk() {
  auto* overview =
  ...
  auto* tile_pixels =
      static_cast<std::uint16_t*>(heap_caps_malloc(2U * vector_v2::kTileBytes, kExternalCaps));
  auto* tile_scratch =
      static_cast<std::uint16_t*>(heap_caps_malloc(vector_v2::kTileBytes, kExternalCaps));
  ...
  auto* strip = static_cast<std::uint16_t*>(heap_caps_malloc(
      static_cast<std::size_t>(vector_v2::kOverviewWidth * kStripRows) * sizeof(std::uint16_t),
      kExternalCaps));
  if (overview == nullptr || raw_slots == nullptr || tile_pixels == nullptr ||
      ...
      strip == nullptr) {
    std::printf(
        "TINYDRAW_VECTOR_V2_WALK_FAIL reason=allocation overview=%u slots=%u tile=%u "
  ...
  if (overview_source == nullptr) {
    std::printf("TINYDRAW_VECTOR_V2_WALK_FAIL reason=overview_source\n");
    return;
  }
  std::copy_n(overview, vector_v2::kOverviewPixels, overview_source);
  if (!canvas.ready() ||
      !canvas.publish_overview({0}, std::span(overview_source, vector_v2::kOverviewPixels)) ||
      !scheduler.ready() || !display.ready() || !touch.ready()) {
    std::printf(
        "TINYDRAW_VECTOR_V2_WALK_FAIL reason=bootstrap canvas=%u scheduler=%u display=%u "
        "touch=%u\n",
  ...
  const std::span strip_pixels(strip,
                               static_cast<std::size_t>(vector_v2::kOverviewWidth * kStripRows));
  ...
  constexpr TileKey kAffected{ZoomLevel::k100Percent, 0, 0};
  constexpr TileKey kCarried{ZoomLevel::k100Percent, 1, 0};
  fill_tile_from_overview(std::span(tile_scratch, vector_v2::kTilePixels), canvas.overview_pixels(),
                          kAffected);
  pass = canvas
             .publish_tile(kAffected, {0}, MaterializationQuality::kSettled,
                           std::span(tile_scratch, vector_v2::kTilePixels))
             .has_value() &&
         pass;
  fill_tile_from_overview(std::span(tile_scratch, vector_v2::kTilePixels), canvas.overview_pixels(),
                          kCarried);
  ...
  pass =
      ...
      append_and_commit_probe(operation_log, canvas, *pen,
                              std::span(overview_source, vector_v2::kOverviewPixels),
                              std::span(tile_scratch, vector_v2::kTilePixels)) &&
  ...
  std::printf(
      "TINYDRAW_VECTOR_V2_WALK_BURST operations=%lu revision=%lu total_us=%lld average_us=%lld "
  ...
  std::printf(
      "TINYDRAW_VECTOR_V2_TOUCH_PROBE ready=%u points=%lu changes=%lu errors=%lu last_x=%.0f "
  ...
  const vector_v2::DisplaySchedulerStats scheduler_stats = scheduler.stats();
  ...
  std::printf(
      "TINYDRAW_VECTOR_V2_WALK_DONE pass=%u panel_rejects=%lu scheduler_accepted=%lu "
...
extern "C" void app_main() { tinydraw::esp32::run_vector_v2_overview_walk(); }
