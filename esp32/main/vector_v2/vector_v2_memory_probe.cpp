...
void run_vector_v2_memory_probe() {
  using namespace tinydraw::vector_v2;
  ...
  for (auto& allocation : allocations) {
    ...
    if (allocation.memory == nullptr) {
      ...
      std::printf("TINYDRAW_VECTOR_V2_MEMORY_ALLOCATION name=%s bytes=%lu ok=0\n", allocation.name,
                  static_cast<unsigned long>(allocation.bytes));
      break;
    }
    allocated_bytes += allocation.bytes;
    std::printf("TINYDRAW_VECTOR_V2_MEMORY_ALLOCATION name=%s bytes=%lu ok=1\n", allocation.name,
                static_cast<unsigned long>(allocation.bytes));
  }
  ...
  std::printf(
      "TINYDRAW_VECTOR_V2_MEMORY_RECEIPT plan_bytes=%lu allocated_bytes=%lu "
  ...
  std::printf("TINYDRAW_VECTOR_V2_MEMORY_DONE pass=%u\n",
