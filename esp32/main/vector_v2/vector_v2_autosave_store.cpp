...
struct VectorV2AutosaveStore::Impl {
  ...
  bool write_pending(const PendingWrite& pending) {
    ...
    if (pending.erase_tail &&
        !erase_range(pending.offset, partition->size - pending.offset)) {
      return false;
    }
    ...
    const std::size_t body_bytes =
        pending.size - vector_v2::kAuthorityJournalCommitMarkerBytes;
  ...
  void worker() {
    for (;;) {
      ...
      if (written) {
        committed.store(pending->sequence);
      ...
      if (write_failed.load()) {
        save_status.store(VectorV2AutosaveStatus::kError);
      } else if (uxQueueMessagesWaiting(queue) == 0U) {
        save_status.store(checkpoint_needed.load() ? VectorV2AutosaveStatus::kNeedsCheckpoint
                                                   : VectorV2AutosaveStatus::kIdle);
      }
    }
  }
  ...
  bool submit(vector_v2::JournalChange change, const vector_v2::OperationLog& log,
    ...
    if (pending == nullptr) {
      checkpoint_needed.store(true);
      save_status.store(VectorV2AutosaveStatus::kNeedsCheckpoint);
      return false;
    }
    ...
    if (offset > partition_bytes || transaction_bytes > partition_bytes - offset) {
      ...
      free_pending(pending);
      save_status.store(VectorV2AutosaveStatus::kFull);
      return false;
    }
    ...
    if (!encoded || xQueueSend(queue, &pending, 0) != pdTRUE) {
      xSemaphoreGive(mutex);
      free_pending(pending);
      checkpoint_needed.store(true);
      save_status.store(VectorV2AutosaveStatus::kNeedsCheckpoint);
    ...
    save_status.store(VectorV2AutosaveStatus::kSaving);
    xSemaphoreGive(mutex);
    return true;
  }
  ...
  std::atomic<VectorV2AutosaveStatus> save_status{VectorV2AutosaveStatus::kIdle};
...
VectorV2AutosaveRestoreStatus VectorV2AutosaveStore::restore(vector_v2::OperationLog& log,
  ...
  auto* records = static_cast<vector_v2::OperationRecord*>(heap_caps_malloc(
      vector_v2::kOperationRecordBytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  auto* samples = static_cast<vector_v2::CompactOperationSample*>(heap_caps_malloc(
      vector_v2::kOperationSampleBytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  ...
  const vector_v2::JournalRecovery recovery = vector_v2::recover_authority_journal(
      source, impl_->partition->size,
      std::span(records, vector_v2::kOperationCapacity),
  ...
  if (recovery.status == vector_v2::JournalRecoveryStatus::kRecovered) {
    const bool restored = log.restore(
    ...
    if (restored && recovery.bytes_consumed % kSectorBytes == 0U) {
      ...
      impl_->committed.store(recovery.sequence);
      impl_->checkpoint_needed.store(recovery.discarded_tail);
      impl_->erase_tail_before_next = recovery.discarded_tail;
      impl_->save_status.store(recovery.discarded_tail
                                   ? VectorV2AutosaveStatus::kNeedsCheckpoint
                                   : VectorV2AutosaveStatus::kIdle);
    ...
    impl_->save_status.store(VectorV2AutosaveStatus::kNeedsCheckpoint);
...
bool VectorV2AutosaveStore::flush(std::uint32_t timeout_ms) {
  ...
  while (impl_->worker_active.load() || uxQueueMessagesWaiting(impl_->queue) != 0U) {
    if (xTaskGetTickCount() - started >= timeout) {
      return false;
    }
    vTaskDelay(pdMS_TO_TICKS(1));
  }
  return impl_->save_status.load() == VectorV2AutosaveStatus::kIdle;
}

VectorV2AutosaveStatus VectorV2AutosaveStore::status() const {
  return impl_ != nullptr ? impl_->save_status.load() : VectorV2AutosaveStatus::kError;
}

std::uint64_t VectorV2AutosaveStore::committed_sequence() const {
  return impl_ != nullptr ? impl_->committed.load() : 0U;
}
