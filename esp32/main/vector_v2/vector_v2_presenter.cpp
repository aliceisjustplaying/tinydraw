...
#else
  // Fill four DMA buffers before requiring the next configured TE edge, then
  // submit one monotonically increasing row-zero sweep from those strips.
  // Two additional free buffers keep every subsequent acquire+stage interval
  // strictly below its wire time.
  const auto sweep = present_ring({0, 0, vector_v2::kOverviewWidth, canvas_bottom}, chrome,
                                  event_us, no_exposed, true);
...
LivePresentationTiming VectorV2Presenter::present_ring(
  ...
  const bool streamed = display_.stream_rect_ring(
      ...
      gate_on_tear_edge ? PanelStreamGate{.context = &context,
                                          .wait = &wait_for_stream_edge_thunk,
                                          .prestage_strips = 4U}
                        : PanelStreamGate{});
  if (!streamed) {
  ...
  const std::int64_t first_submitted =
      gate_on_tear_edge ? timing.tear_edge_observed_at_us : stream_started;
