...
struct TileClassCensus {
  ...
  void include(const production::TilePayloadAnalysis& analysis, std::int64_t elapsed_us) {
    ++tiles;
...
void print_zoom(ZoomLevel zoom, const TileClassCensus& stats, bool passed) {
  ...
  std::printf(
      ...
      static_cast<unsigned long>(stats.raw * production::kTileBytes),
...
bool run_production_tile_census(production::TileProducer& producer, MaterializedCanvas& canvas,
  ...
  bool passed = packed_scratch.size() >= production::kTilePixels;
  for (const ZoomLevel zoom : zooms) {
    TileClassCensus stats{};
    passed = passed && canvas.discard_tiles();
    const production::TileGrid grid = production::tile_grid(zoom);
    for (int group_row = 0; group_row < grid.rows && passed;
         group_row += production::kTileProducerRows) {
      for (int group_column = 0; group_column < grid.columns && passed;
           group_column += production::kTileProducerColumns) {
        const int last_column =
            std::min(grid.columns, group_column + production::kTileProducerColumns);
        const int last_row = std::min(grid.rows, group_row + production::kTileProducerRows);
        const production::PixelRect first_bounds =
            production::tile_pixel_bounds({zoom, static_cast<std::uint16_t>(group_column),
                                           static_cast<std::uint16_t>(group_row)});
        const production::PixelRect last_bounds =
            production::tile_pixel_bounds({zoom, static_cast<std::uint16_t>(last_column - 1),
        ...
        for (int row = group_row; row < last_row && passed; ++row) {
          for (int column = group_column; column < last_column && passed; ++column) {
            ...
            const production::PixelRect bounds = production::tile_pixel_bounds(key);
            ...
            const std::int64_t started = esp_timer_get_time();
            const auto analysis = production::analyze_tile_payload(pixels, width, height);
  ...
  std::printf(
      ...
      static_cast<unsigned long>(total.tiles * production::kTileBytes),
      static_cast<unsigned long>(total.raw * production::kTileBytes),
