...
// A fixed 3x3 drawing world around one screen-sized raster viewport. Full-world
// copies stay on the pan path; normal stroke updates continue using StrokeRaster.
class WorldCanvas {
 public:
  static constexpr int kWidth = kCanvasWidth * 3;
  static constexpr int kHeight = kCanvasHeight * 3;
 ...
 private:
  ...
  ViewOrigin origin_{(kWidth - kCanvasWidth) / 2, (kHeight - kCanvasHeight) / 2};
  bool valid_ = false;
};
