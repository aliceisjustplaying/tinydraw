...
release timing drives the firmware's short HUD-toggle and long demo-record controls.
...
The stripped `-Oz` reactor is currently 230,116 bytes (about 225 KiB). It
...
converts panel payload bytes to a raw 40 MHz quad-SPI wire floor. It does not
model instruction execution, cache behavior, SRAM or PSRAM latency, flash
traffic, DMA queue overlap, or panel scan-out, so its cycle field is not a
total execution-cycle claim. `puck/timing_verify.ts` checks two fresh instances
byte-for-byte and pins the startup transfer at 329,728 bytes, 11 submissions,
659,456 panel wire clocks, and a 3,956,736-cycle CPU-clock equivalent without
changing the tolerance-zero pixel traces.
