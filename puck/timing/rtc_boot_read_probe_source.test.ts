...
test("RTC boot-register peers define matched 2048- and 4096-read cohorts", async () => {
  ...
  expect(firmware).toContain("prime < PrimeCount");
  expect(firmware).toContain("esp_rom_delay_us(5'000U)");
  expect(firmware).toContain("measure_rtc_date_boot_4096_once");
  expect(firmware).toContain("measure_rtc_xtal_boot_4096_once");
