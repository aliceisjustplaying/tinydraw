...
Frozen: 2026-08-15, by product author decision. Changes require an explicit
author decision recorded here with a date; nothing below drifts silently.

Stable product priorities live in
[`PRODUCT_TENETS_PRE_RELEASE_2026-08-19.md`](PRODUCT_TENETS_PRE_RELEASE_2026-08-19.md).
...
Status: correctness provisionally GREEN on the final invariant build (author
...
full-frame ([hardware receipt](../../receipts/hardware/CO5300_PANEL_LIMITS_2026-08-15.md)). Pan-tool drags that start on canvas overlays
must enter pan after an 8 px intent threshold; stationary overlay taps retain
their control action (reported zoom-rail swallowing defect, fixed and
gated 2026-08-17).
...
**Priority order per author: the déjà vu (re-rendering already-rendered
...
- **Exact variable-width Perfect-Freehand fidelity. Centerline/uniform-width
  export is forbidden** (author: "absolutely no").
...
- By author decision on 2026-08-17, authority (operations + samples +
...
- Power loss loses at most the in-progress gesture plus ≤5 s of committed
  work. (PROPOSED default — author has not reviewed this number.)
...
- Onboard clock + one-shot NTP: required (export timestamps). V2's on-demand
  ...
  author accepted unavailable-network handling, successful RTC sync, centered
  terminal feedback, and text size on glass
  (`benchmark-results/v2-ntp-sync-2026-08-17/RECEIPT.md`).
- Minimap: tap-to-jump and viewport-drag navigation required (author elevated
  ...
  and glass acceptance
  (`benchmark-results/minimap-absolute-pointer-2026-08-17/RECEIPT.md`).
- Zoom: 25/50/100/200/400%. By author decision on 2026-08-17, transitions keep
...
Author decision, 2026-08-16: V2 documents are **blank baseline plus ordered
...
## Author decisions — 2026-08-16 (post Cold Stage B glass session)
...
1. **Mixed-draw append lag is a defect, not a budget problem.** The felt
   ...
   `docs/archive/2026-08-code-reviews/2026-08-18-performance-review/EXTERNAL_REVIEW_SYNTHESIS_2026-08-16.md`
   item 9), adopted as the
...
3. **Stage C authority bundle declined.** Conical capsules + adaptive
   ...
   tool falsified the smoothness case on recorded recorded touch input
...
4. **Settled-AA prototype approved.** The boundary-only analytic-coverage
   ...
   before/afters. On-device go/no-go after author review of the prototype.
5. **Overlap-50 cold red is binding and sequenced (2026-08-16).** The
   ...
   the settled-AA prototype review, and the déjà-vu fix. Author was
...
## Author decisions — 2026-08-17 cleanup closure
...
- Demo record/replay (author: "rather nice — after shipping").
...
8. Every red flag in the harness verdict vector must have a scorecard row
   in `PROJECT_STATE.md` naming its number, its threshold, and its author
