...
[`REVIEW.md`](../archive/2026-08-code-reviews/review-findings/2026-08-16-cold-campaign/REVIEW.md),
[`HANDOVER.md`](../archive/2026-08-code-reviews/review-findings/2026-08-16-cold-campaign/HANDOVER.md),
...
- Compute must fall **582 → ~413 ms** for the ≤500 ms required gate,
  ...
  ([`SHIP_CONTRACT.md:46`](../../SHIP_CONTRACT.md)).
...
6. **§4.5/4.6 conical capsule authority** — not present anywhere in the
   ...
   in the review. Owner-gated (authority version bump). Adopt as an A/B —
   see §4 sequencing.
...
8. **§8.1 dishonest budget comment** — `incremental_document.cpp:305–307`
   ...
   counters. This also directly serves the open `mixed_draw` 18.8 ms owner
...
10. **§9.2 trace corpus is synthetic** — confirmed: all four committed
    ...
    (`testdata/ink-traces/`). Both reviews agree: record the owner corpus
...
12. **§9.5 selective 1/2/4 subdivision** — converges with the internal
    owner-gated four-span rematch (wave-3 destroyed the old cost model).
...
## 3. The one real divergence needing an owner decision: dual-core
...
- A gated, receipt-driven pipeline with a stop rule is arguably not
  "speculative," but relaxing the guardrail is the owner's call, not
...
owner traces** (10). The format library exists
...
is a hard prerequisite for the Stage C evidence below. Needs an owner
...
envelope on the recorded owner traces (host only); produce side-by-side
...
**Stage C — owner gate: bundled authority version bump (this IS the ink
smoothness stage)**
If Stage B leaves the wall >500 ms (likely), take ONE authority change
event to the owner combining:
...
an explicit old-vs-new bridge measurement, owner glass acceptance of the
...
pays the re-baseline twice. The owner decision is made with both inputs
...
`mixed_draw` owner decision with data instead of a budget edit.
...
4. Settled AA prototype go (owner already said "need to see it in
