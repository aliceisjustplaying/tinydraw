...
TEST_CASE("tile payload analysis rejects malformed input") {
  ...
  CHECK_FALSE(production::analyze_tile_payload(pixels, 0, 4));
  CHECK_FALSE(production::analyze_tile_payload(pixels, 2, 0));
  CHECK_FALSE(production::analyze_tile_payload(pixels, production::kTileWidth + 1, 1));
  CHECK_FALSE(production::analyze_tile_payload(pixels, 2, 3));
}
...
TEST_CASE("uniform tile has one row run per row") {
  ...
  const auto analysis = production::analyze_tile_payload(pixels, 4, 3);
...
TEST_CASE("row run estimate resets at every row") {
  ...
  const auto analysis = production::analyze_tile_payload(pixels, 4, 2);
  REQUIRE(analysis.has_value());
