...
3. build the exclusive live-touch sample collector around the extracted `PhysicalTouch` transport and `append_incrementally`, without moving production policy into `hardware_app.cpp`;
...
The current mutation receipt is [`production/hardware-receipts/acaa8a6-mutation-burst-walk.log`](production/hardware-receipts/acaa8a6-mutation-burst-walk.log). Thirty additional ordered pen/eraser appends advanced log and canvas together to revision 32 in 1.435 seconds total (47.840 ms average), then produced the expected `d4e162c4` panel hash with 168/168 transfers and zero rejects. That is below the <100 ms first-feedback product gate, although the full live path remains unmeasured. The 48 ms cost is dominated by copying the 329,728-byte overview into publication storage and committing it back, not tile composition. The CST820 transport is now separated from the legacy app and the exclusive production image initializes it successfully. The exact-commit [`production/hardware-receipts/67314b8-touch-ready-walk.log`](production/hardware-receipts/67314b8-touch-ready-walk.log) recorded a bounded idle probe with zero I2C/read errors, but no finger was present, so it is only a readiness receipt—not live-input evidence. These results do not prove representative operation-log capacity, live touch integration, settled anti-aliasing, concurrent scheduling, or interactive pan.
