...
| Minimap navigation | **Fine-control fix installed; glass pending** | The no-draw guard still extends around the frame to panel edges/toolbar. Owner glass testing rejected the 400% mapping: its invisible 44×44 preserve-offset zone made touches outside the visible ~5×5 viewport appear inactive, while 2 px of finger movement jumped ~146 level pixels. Only the truthful viewport now preserves offset; every other minimap start directly acquires it, and high-zoom deltas use the 100% map scale (2 px → 36 level px at 400%). The focused repro went four-red → four-green, and the physical classifier reports old-zone acquisition, center acquisition, and `pass=1` ([fine-control receipt](benchmark-results/minimap-fine-control-2026-08-17/RECEIPT.md), [input-leak receipt](benchmark-results/minimap-input-leak-2026-08-17/RECEIPT.md)). |
| RTC / one-shot NTP | **Network flow accepted; larger text installed, glass pending** | Document → Clock launches a low-priority asynchronous connect/NTP/RTC-write attempt with modal `CONNECTING`/`SYNCING` and terminal `TIME SET`/`TIME ERROR` feedback. The owner confirmed unavailable-network handling terminates, a successful sequence reaches `TIME SET`, and labels are centered. All NTP labels now match export `SAVING` at scale 3; the NTP-only toast widens symmetrically to fit `CONNECTING`. The focused pixel test went red at 14 vs 21 px and now passes 16 assertions; the combined normal product booted `pass=1` ([text-size receipt](benchmark-results/ntp-text-size-2026-08-17/RECEIPT.md), [NTP receipt](benchmark-results/v2-ntp-sync-2026-08-17/RECEIPT.md)). Credentials stay in the ignored local header; timezone is UTC. Final size acceptance and a serial-captured second network attempt remain. |
