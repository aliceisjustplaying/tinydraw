...
| Ink latency | **Provisional green — visual lane and lift; smoothness yellow** | The five-trace canonical corpus is recorded owner finger input replayed through the production `offer()` path: zero lost Down/Up, event→DMA p95 2.3–5.3 ms on every trace ([`BASELINE.md`](benchmark-results/ink-trace-replay-baseline/BASELINE.md)). The committed overlay closed the lift hitch on glass: 87–199 ms → 10–34 ms, drain-gated to ~5 ms expected ([`RECEIPT.md`](benchmark-results/committed-overlay/RECEIPT.md)). Smoothness: the angularity tool falsified the four-span case on real input; the signal points at arc-length resampling + settled AA ([`BASELINE.md`](benchmark-results/ink-angularity-baseline/BASELINE.md)). The formal optical-latency receipt remains open. |
