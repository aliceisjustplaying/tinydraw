...
| Autosave / recovery | **Green — owner glass accepted** | V2 journals vector authority and session state: retained Redo, active prefix, generation/epoch, complete remembered navigation, tool/size/color, and next Stroke identity. Consecutive queued state-only changes at one authority revision coalesce before the flash worker claims them; authority changes remain distinct transactions. Host fixtures preserve the prior Recovery point after truncation and corruption, and the complete 31-target host suite and all 13 ASan/UBSan targets pass. Physical recovery restored blank and real multi-Stroke documents with camera, selections, and Redo intact. Two-arena compaction remains deferred; a full journal preserves existing commits and reports capacity. |
