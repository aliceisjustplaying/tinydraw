...
1. capture a representative real operation document and use it to validate the provisional 4,000-operation, 80,000-source-sample, and 90,000-LOD-sample capacities; the checked-in synthetic characterization is sizing guidance only;
2. decide the approximately 50 ms transactional overview-copy path before adding settled work to every append: measure dirty-rectangle publication versus double-buffer ownership rather than optimizing by assumption;
...
4. select a zoom-specific LOD policy only after comparing captured-input screen-space error and rendered output at 50–400%; `OperationLodStore` is ready, but no default simplifier is approved;
