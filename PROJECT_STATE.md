...
| Settled AA | **Green — owner accepted for release scope** | Idle settle republishes tiles at the settled quality tier (revisit-ledger-safe); 25% settles presentation pixels only because the overview remains hard-edged replay authority. Per-tile work is 1.7–5.4 ms mean / 9.3 ms max after the annulus + batching round (was 5–11/17), and the full battery moved zero gates. The owner accepted AA as done for the current release scope on 2026-08-17. Faster progression and cosmetic tuning may return later but are optional rather than blockers. SVG exports exact continuous ribbon outlines that viewers anti-alias when rasterizing; device settled-AA pixels remain a presentation derivative rather than path authority. |
...
| Minimap navigation | **Red — owner rejected latest 400% mapping** | The no-draw guard still protects the frame, but glass testing rejected commit `54dc125` as worse. From the bottom-right, acquisition took roughly five attempts; after acquisition, the new 0.25 high-zoom delta scale made the finger run off the physical minimap before the viewport could return to center. The deterministic classifier stayed green because it checks computed endpoints, not real acquisition or finite finger travel. The prior 44×44 hidden-zone ambiguity was real, but replacing it and slowing all drag deltas did not solve the product interaction. A non-perturbing exact minimap capture path is required before another mapping change; the existing synchronous CSV dump is disqualified ([fine-control receipt](benchmark-results/minimap-fine-control-2026-08-17/RECEIPT.md), [capture diagnosis](benchmark-results/owner-glass-capture-2026-08-17/RECEIPT.md)). |
