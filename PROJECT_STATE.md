...
| SVG export | **Stroke grouping flashed; owner export follow-up pending** | The owner opened a physical `DRAWING.SVG` in Inkscape and found the remaining semantic mismatch: bounded chunks from one finger stroke exported as separate paths, plus one synthetic white background rectangle. Adjacent chunks already preserve one nonzero gesture ID, so export now streams one painter-ordered filled path per physical Stroke and omits the rectangle without adding document-sized storage. The mounted pre-fix artifact measured 98,456 bytes, 24 paths, and one rect; focused SVG tests now pass 13,443 assertions ([original device receipt](benchmark-results/svg-export-2026-08-17/RECEIPT.md), [follow-up receipt](benchmark-results/stroke-svg-minimap-acquire-2026-08-17/RECEIPT.md)). |
