...
owner-accepted absolute minimap control, and on-demand V2 NTP UI landed;
receipts:
...
| Exactness | **Undo/Redo software green; device gate pending** | The active/retained operation prefixes are one generation-checked authority. Whole-Stroke Undo/Redo spans adjacent chunks, preserves ≥10 levels, replaces Redo only when new ink publishes, and replays bounded damage transactionally. Mixed pen/eraser tests restore a line cut by an eraser byte-for-byte; producer rebuild and unaffected-tile retention are covered. All 29 host targets and both credentials-enabled ESP builds pass. Physical multi-zoom, latency, and export-after-history checks remain; persistence is the next feature. |
...
| Feature parity | **Open** | Undo/Redo software integration is green with its device gate pending. Autosave/recovery, power lifecycle parity, remaining failure UI, and release soak remain. |
...
2. **Implemented; physical gate pending:** the authority spine and whole-Stroke
   Undo/Redo are host- and compile-green, including mixed pen/eraser replay,
   branch replacement, localized damage, and ≥10 levels.
3. **Next:** complete all autosave work: versioned journal, bounded idle writes,
