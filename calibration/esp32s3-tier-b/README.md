...
eim run "idf.py -C calibration/esp32s3-tier-b \
  -B out/build/esp32s3-tier-b build"
...
```text
eim run "idf.py -C calibration/esp32s3-tier-b \
  -B out/build/esp32s3-tier-b -p PORT flash"
uv run --script tools/tier-b-capture.py \
  PORT ~/Archives/esp32s3/tier-b/boot-1/serial.log 900 \
  --variant normal \
...
```text
python3 tools/tier_b_ndjson.py ~/Archives/esp32s3/tier-b/boot-1/serial.log \
